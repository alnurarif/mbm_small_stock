<script setup>
    ...
    const showEditReceive = (id) => {
        ...
        let single_receive = receives.value.find(receive => receive.id == id);
        form.value = {
    ...
    const showDetailReceive = (id) => {
        showDetailModal();
        singleReceiveDetail.value = receives.value.find((receive) => receive.id === id);
    }
		...
		<div class="content">
            ...
            <table>
                ...
                <tbody>
                    ...
                    <tr v-for="(receive, index, key) in receives" :key="receive.id" v-if="receives.length > 0">
                        <td>{{index + 1}}</td>
                        <td>{{receive.supplier.name}}</td>
                        <td>{{receive.item.name}}</td>
